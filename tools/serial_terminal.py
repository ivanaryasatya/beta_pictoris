...
def write_to_csv(file_path, time, msg_type, content):
    """Fungsi untuk mencatat data ke file CSV"""
    with open(file_path, mode='a', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([time, msg_type, content])
        ...
        try:
            if ser.in_waiting > 0:
                ...
                if data:
                    ...
                    print(f"\r[{timestamp}] [RX] -> {data}")
                    print(">> ", end="", flush=True)
                    # Catat ke CSV
                    write_to_csv(log_file, timestamp, "RX", data)
...
def main():
    ...
    if not os.path.exists(LOG_DIR):
        os.makedirs(LOG_DIR)
        print(f"[SISTEM] Folder log dibuat: {LOG_DIR}")
    ...
    try:
        ...
        with open(full_log_path, mode='w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["time", "jenis komunikasi", "log"])
        
        print(f"\n[SISTEM] Terhubung ke {port}")
        print(f"[SISTEM] Logging ke: {full_log_path}")
        print("[SISTEM] Ketik pesan lalu Enter. Ketik 'exit' untuk keluar.\n")
        ...
        while True:
            ...
            if tx_data:
                ...
                # Catat ke CSV
                write_to_csv(full_log_path, timestamp, "TX", tx_data)
